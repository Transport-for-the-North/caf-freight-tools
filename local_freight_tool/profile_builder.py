...
class Profile_Builder(QtWidgets.QWidget):
    ...
    def initUI(self):
        ...
        self.outpath = Utilities.add_file_selection(
            self, 20+22*(10+14), "Select the output directory:", directory=True, box_width=430
        )
    ...
    def go_button_clicked(self, state):
        if not self.outpath.text().strip():
            ...
            df.dropna().to_csv(self.outpath.text().strip()+'/Profile_Selection.csv', index=False, header=False)
